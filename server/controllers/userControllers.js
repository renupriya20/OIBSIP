      ...
      if (user) {
        ...
        if (password !== confirmPassword) {
          ...
          if (password.length < 8) {
            ...
            // Send the verification email — wrapped safely so a failed email
            // (bad SMTP creds, network issue, etc.) never crashes registration
            let emailSent = false;
            try {
            ...
            // User is created regardless of whether the email went through,
            // so a broken SMTP config never blocks account creation.
            user = await User.create({
              name,
              email,
              password: hashedPassword,
            ...
            res.status(200).json({
              ...
              message: emailSent
                ...
                : 'User Registered Successfully! (Verification email could not be sent — please check your email configuration.)',
            });
          }
        }
      }
  ...
  if (!email) {
    ...
    if (emailValidator.validate(email)) {
      ...
      if (user) {
        ...
        // Send the reset email — wrapped safely so a failed email doesn't 500
        let emailSent = false;
        try {
          emailSent = await sendEmail({
            to: user.email,
        ...
        if (emailSent) {
          ...
          res.status(400);
          throw new Error(
            'Error Sending Password Reset Email! Please check your email configuration.'
          );
        }
      } else {
        res.status(400);
          ...
          if (user.resetPasswordToken === resetToken) {
            if (newPassword !== confirmNewPassword) {
              ...
              if (newPassword.length < 8) {
                ...
                const salt = await bcrypt.genSalt(10);
                const hashedPassword = await bcrypt.hash(newPassword, salt); // FIXED: added missing 'const'
